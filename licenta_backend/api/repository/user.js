...
async function getStudents(id) {
    return db.queryPromise('SELECT first_name,last_name,users.address,birthday,email,faculties.name FROM users JOIN faculty_members ON users.id = faculty_members.user_id JOIN faculties ON faculty_members.faculty_id = faculties.id WHERE users.role_id = 6 AND faculties.id = ?;', [id]);
}
